...
my @dtafiles = glob("/home/bxie/yeast4plex/yeast_RP_POS_ph3_HM_01/yeast_RP_POS_ph3_HM_01.1/*.MS1");
#my @dtafiles = ("/home/bxie/yeast4plex/yeast_RP_POS_ph3_HM_01/yeast_RP_POS_ph3_HM_01.1/10090.MS1");
my $p = Spiders::Params->new('-path'=>$parameter);
my $params = $p->parse_param();
my $database = $params->{'database'};
my $decoy_database = $database . "_decoy";
...
foreach(@dtafiles)
{
	...
	my $command = "wc -l $dtafile";
	my @out = qx($command);
	my $nLines = (split(/\s+/, $out[0]))[0];
	next if ($nLines > 101);

=head
	$command = "wc -l $dtafile.pair.all.comb";
	@out = qx($command);
	my $nLines = (split(/\s+/, $out[0]))[0];
	next if ($nLines < 20);

	$command = "wc -l $dtafile.smout";
	@out = qx($command);
	my $nLines = (split(/\s+/, $out[0]))[0];
	next if ($nLines < 5);
=cut
...
#    print RESULT "Index\tC12 mass\tN15 mass\tC13 mass\tMS1 Scan\tC12 Intensity\tN15 Intensity\tC13 Intensity\tPscore\tFormula\tAdduct\tName\tStructure (SMILES)\tInChI\tType\n";
	...
	my $nClusterPeaks = scalar(keys %$cluster);
	next if ($nPeaks == $nClusterPeaks);
